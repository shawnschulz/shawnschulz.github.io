...
var camera = new THREE.PerspectiveCamera( 45, 80/ 80  );

    camera.position.set(0, 0, 0);
    camera.lookAt( scene.position );
var renderer = new THREE.WebGLRenderer( {antialias: true, alpha: true} );
renderer.setSize( 80, 80);
renderer.domElement.style.padding = '10px';
renderer.domElement.style.boxSizing = 'border-box';

container.insertBefore( renderer.domElement, container.firstChild );

scene.background = new THREE.Color(0x44735F);
// Lighting
    ...
    './src/stromal_like.glb', (gltf) => {
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                child.material.color.setHex(0xFFFFFF);   // or any color
